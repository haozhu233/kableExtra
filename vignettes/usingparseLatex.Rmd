...
Currently the code using `parseLatex` uses very similar arguments to the 
original `kableExtra` code, but in the long run the internal functions should not 
keep parsing and deparsing, they should work directly on the `parsedInput` object.
